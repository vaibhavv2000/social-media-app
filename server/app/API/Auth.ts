...
router.post("/register",async (req: Request,res: Response,next: NF) => {
  ...
  try {
    ...
    // const newUser = await sql.execute(`CALL new_user(?, ?, ?, ?)`,[
    //   name,
    //   username,
    //   email,
    //   hash,
    ...
    if(mobile) {
      // @ts-ignore
      return res.status(201).json({user: newUser[0][0]});
    }
...
router.post("/login",async (req: Request,res: Response,next: NF) => {
  ...
  try {
    ...
    if(mobile) {
      return res.status(200).json({user: n_user,aT});
    }
...
router.delete("/deleteuser",isAuth,async (req: Request,res: Response,next: NF) => {
